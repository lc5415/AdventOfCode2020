...
# %%
# lookup of bags to contents
function input2dict(data_in) 
    lookup = Dict()
    for sentence in split(data_in, '\n')
        split_sen = split(sentence, " contain ")
        left, right = split_sen[1], split_sen[2]
        left = replace(left, r" bags| bag" => "")
        right = replace(right, r" bags| bag" => "")
        right = replace(right, "." => "")
        right = split(right, ", ")
       if any(contains(r, "other") for r in right)
            lookup[left] = Dict("empty" => 0)
        else 
            lookup[left] = Dict(i[3:end] => parse(Int64, i[1]) for i in right)
        end
    end
    return lookup
end
...
# %%
lookup = input2dict(input)

# %%
expanded_lookup = Dict(bag => [] for bag in keys(lookup))

# need to assign to mother bag list all the bags of the children
count = 0
function dig(motherbag, childbag = "")
#     print(childbag, '\n')
#     [print('\t', i, '\n') for i in lookup[childbag]]
    if haskey(lookup[childbag], "empty")
        return
    else
        expanded_lookup[motherbag] =
            cat(expanded_lookup[motherbag],
            collect(keys(lookup[childbag]));dims = 1)
        
        for ch in keys(lookup[childbag])
            dig(motherbag, ch)
        end
    end
end

for (bag, content) in lookup
    dig(bag, bag)
end

can_contain_gold = 0
for (item, content) in expanded_lookup
    ## pretty print
    #print(item, '\n')
    #[print('\t', i, '\n') for i in content]

    if any("shiny gold" == i for i in content)
        global can_contain_gold += 1
    end
end
...
# %%
ex2 = "light red bags contain 1 bright white bag, 2 muted yellow bags.
...
# %%
function dig(rootbag, current_bag = "", current_count = 1)  
    #@printf("%s, %s, %f\n", rootbag, current_bag, current_count)
    if haskey(lookup[current_bag], "empty")
        return
    else
        ## 
        count_lookup[rootbag] += 
            current_count*sum(v for v in values(lookup[current_bag]))
        
        for (child, num) in lookup[current_bag]
            #@printf("%s: %f\n", child, num)
            dig(rootbag, child,
                num*current_count) # the num of the current bag 
            # is chained with the current count which starts at 1
            # the branch counting goes like:
            # n1 = number of current bags (1) * number of children bag in current bag
            # n2 += n1 * number of children bag in current bag (2)
            # n3 += n2 * number of children bag in current bag(3)
            # n4 ... until empty bag is found,
            # then branches are just stacked (summed) on top of each other
        end
    end
end
...
# %%
lookup = input2dict(ex)
count_lookup = Dict(bag => 0 for bag in keys(lookup));
for bag in keys(lookup)
    dig(bag, bag, 1)
end
print(count_lookup["shiny gold"], '\n')

# %%
lookup = input2dict(ex2)
count_lookup = Dict(bag => 0 for bag in keys(lookup));
for bag in keys(lookup)
    dig(bag, bag, 1)
end
