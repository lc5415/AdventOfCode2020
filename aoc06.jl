...
## Part 1 alternative
# I could have used Sets, which have the feature of returning every item 
# in a collection once and only once, this would have been more optimal than going
# thorugh all letters a to z
uniques = 0
for group in split(input, "\n\n")
	group = replace(group, '\n' => "")
	unique_letters = Set(group)
	global uniques +=length(unique_letters)
end
print(uniques, '\n')
...
uniques = 0
for group in split(input, "\n\n")
	# count number of people == number of newlines + 1
	n_people = count(i->(i=='\n'), group)+1	
	group = replace(group, '\n' => "")
	for letter in "abcdefghijklmnopqrstuvwxyz"
		c_letter = count(i->(i==letter), group)	
		if c_letter == n_people
			global uniques += 1
		end
	end
end
